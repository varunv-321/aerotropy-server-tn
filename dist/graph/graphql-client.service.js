...
let GraphqlClientService = GraphqlClientService_1 = class GraphqlClientService {
    configService;
    logger = new common_1.Logger(GraphqlClientService_1.name);
    apolloClient;
    constructor(configService) {
        this.configService = configService;
        const subgraphUrl = this.configService.get('UNISWAP_V3_BASE_SUBGRAPH_URL');
        const apiKey = this.configService.get('GRAPH_API_KEY');
        if (!subgraphUrl) {
